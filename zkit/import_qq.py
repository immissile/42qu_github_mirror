...
import sys
from bot_txt import txt_wrap_by, txt_wrap_by_all
reload(sys)
...
class PYQQ:
    ...
    def get_friends(self):
        get_online_pages = self.httpRequest('get', 'http://q16.3g.qq.com/g/s?sid=%s&aid=nqqchatMain&on=1&g_f=1655'%self.sid).decode('gb18030')
        maxPage = int(txt_wrap_by('/', '页', txt_wrap_by('下页</a><br/>', '<', get_online_pages)))
        friends = []
        for x in range(maxPage):
            tmp = self.httpRequest('post', 'http://q32.3g.qq.com/g/s?sid=%s&aid=nqqchatMain&on=1&p=%s'%(self.sid, str(x)))
            friends.extend(txt_wrap_by_all('&amp;u=', '&amp;', tmp))

        get_online_pages = self.httpRequest('get', 'http://q16.3g.qq.com/g/s?sid=%s&aid=nqqchatMain&on=0&g_f=1655'%self.sid).decode('gb18030')
        maxPage = int(txt_wrap_by('/', '页', txt_wrap_by('页</a><br/>', '<', get_online_pages)))
        for x in range(maxPage):
            tmp = self.httpRequest('post', 'http://q32.3g.qq.com/g/s?sid=%s&aid=nqqchatMain&on=0&p=%s'%(self.sid, str(x)))
            friends.extend(txt_wrap_by_all('&amp;u=', '&amp;', tmp))
        friends = list(set(friends))
        return friends

def get_qq_friend_list(username, password):
    QQ = PYQQ(username, password)
    QQ.login()
    return QQ.get_friends()

if __name__ == '__main__':
    print get_qq_friend_list('316293191', 'pss')
