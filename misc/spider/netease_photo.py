...
PHOTO_163_HOTLIST_URL='http://photo.163.com/share/dwr/call/plaincall/PictureSetBean.getPictureSetHotListByDirId.dwr?callCount=1&scriptSessionId=%24%7BscriptSessionId%7D187&c0-scriptName=PictureSetBean&c0-methodName=getPictureSetHotListByDirId&c0-id=0&c0-param0=number%3A40&c0-param1=number%3A18&c0-param2=number%3A18&c0-param3=string%3AWeight2&c0-param4=number%3A1&batchId=694311'
...
c0-methodName=getLikeViewCount
c0-id=0"""
        num=len(photos)
        for i in range(num):
                data=data+'\nc0-e'+str(i+1)+'=number:'\
        ...
        for i in range(1,num):
                data=data+',reference:c0-e'+str(i+1)
        data=data+']'

        for i in range(num):
                data=data+'\nc0-e'+str(num+i+1)+'=number:'\
                                +str(photos[i][1])
        data=data+'\nc0-param1=Array:[reference:c0-e'+str(num+1)
...
def photo_163_parse_photo_album(data,url,u_name,a_id,u_id,hits):
		...
		print '\n',url
		print title,published,author,author_url,place,hits,u_id,u_name,a_id
